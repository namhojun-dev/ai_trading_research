...
function scoreColor(s: number): string {
  ...
  if (s >= 55) return "text-cyan-300";
  if (s >= 35) return "text-amber-300";
  return "text-[var(--color-muted)]";
}

export function ScreenerTable({ rows }: { rows: ComparableCompany[] }) {
  if (rows.length === 0) {
    return (
      ...
      <table className="w-full min-w-[920px] border-collapse text-right text-xs">
        <thead>
          <tr className="border-b border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-muted)]">
            ...
            <th className="px-2 py-2">저평가점수</th>
        ...
        <tbody>
          {rows.map((c) => (
            ...
            >
              ...
              <td className={`px-2 py-2 font-mono font-bold ${scoreColor(c.valuationScore)}`}>
